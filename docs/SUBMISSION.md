...
当前主 Demo 聚焦即时零售：当用户处于晚间加班、低能量、天气下雨、临时缺少生活用品等状态时，Mira 会结合公开 mock 用户偏好、附近 mock 库存、mock 配送时效和预算约束，生成一个常用补给包方案。由于涉及金钱和敏感上下文，Mira 必须先通过 policy gate 并等待用户确认，之后才会生成 mock 订单和履约状态。

项目同时提供餐饮推荐、日程出行提醒和预算管理等扩展场景，展示 AI 从“更聪明的搜索框”走向“会主动安排生活节奏的本地生活管家”的可能性。
